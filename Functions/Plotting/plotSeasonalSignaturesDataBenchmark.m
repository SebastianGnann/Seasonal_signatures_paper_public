function [] = plotSeasonalSignaturesDataBenchmark(A,phi,w,varargin)
...
% Tailored function that plots benchmark and example catchments. 
% Options:
...
f1 = figure('Name',attribute_name,'NumberTitle','off','pos',[10 10 350 300]);
hold on
% grid on
...
% disp('No confidence intervals shown.')
s1 = scatter(A,phi,25,attribute,'MarkerEdgeColor','k');%
s2 = scatter(A(attribute),phi(attribute),25,attribute(attribute),...
    'Filled','MarkerFacecolor',[0.7 0.7 0.7],'MarkerEdgeColor','k');%
s3 = scatter(A([1157 586]),phi([1157 586]),25,attribute([1157 586]),...
    'Filled','MarkerFacecolor',colour_mat(8,:),'MarkerEdgeColor','k');%
xlim(x_limits)
ylim(y_limits)
...
leg = legend([s1,s2,s3],'All','UKBN2','From Fig.4');
legend boxoff
